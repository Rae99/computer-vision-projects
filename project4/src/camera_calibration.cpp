...
bool runCameraCalibration(
    ...
    if (pointList.size() != cornerList.size()) {
        std::cout << "pointList size does not match cornerList size." << std::endl;
        return false;
    }

    cameraMatrix = (cv::Mat_<double>(3, 3) <<
        1.0, 0.0, static_cast<double>(imageSize.width) / 2.0,
        0.0, 1.0, static_cast<double>(imageSize.height) / 2.0,
        0.0, 0.0, 1.0
    );
