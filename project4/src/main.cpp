 ...
 * Project4-main.cpp
 * Runs the camera calibration workflow for Project 4.
 * It captures checkerboard samples, performs calibration, and writes
 * the intrinsic result file used by later AR tasks.
 */
...
/**
 * @brief Interactive entry point for camera calibration.
 */
int main() {
    ...
    std::string calibrationResultFile =
    ...
    while (true) {
        ...
        if (found) {
            ...
            cv::putText(outputFrame, "Checkerboard not found",
                        cv::Point(20, 30), cv::FONT_HERSHEY_SIMPLEX, 0.8,
                        cv::Scalar(0, 0, 255), 2);
        }

        std::string savedText =
            "Saved calibration frames: " + std::to_string(corner_list.size());
        cv::putText(outputFrame, savedText, cv::Point(20, 100),
                    cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 255, 0), 2);

        cv::putText(outputFrame,
                    "Press s = save, c = calibrate, w = write file, q = quit",
                    cv::Point(20, 135), cv::FONT_HERSHEY_SIMPLEX, 0.55,
                    cv::Scalar(255, 255, 255), 1);

        if (calibrationReady) {
            std::string errorText =
                "Reprojection error: " + std::to_string(reprojectionError);
        ...
        if (key == 'q') {
            ...
            if (hasLastSuccessfulDetection) {
                ...
                if (point_set.size() != lastSuccessfulCorners.size()) {
                    std::cerr << "Error: point_set size does not match "
                                 "corner_set size."
                              << std::endl;
                    continue;
                }
                ...
                bool imageSaved = saveCalibrationImage(
                    lastSuccessfulFrame, calibrationImageFolder,
                    static_cast<int>(corner_list.size()));

                std::cout << "\nSaved calibration sample " << corner_list.size()
                          << std::endl;
                std::cout << "corner_list size = " << corner_list.size()
                          << std::endl;
                std::cout << "point_list size = " << point_list.size()
                          << std::endl;
                std::cout << "last corner set size = "
                          << corner_list.back().size() << std::endl;
                std::cout << "last point set size = "
                          << point_list.back().size() << std::endl;
                ...
                if (!point_list.back().empty()) {
                    std::cout << "First 3D point = (" << point_list.back()[0][0]
                              << ", " << point_list.back()[0][1] << ", "
                              << point_list.back()[0][2] << ")" << std::endl;
                }

                if (imageSaved) {
                    std::cout << "Saved calibration image to "
                              << calibrationImageFolder << std::endl;
                } else {
                    std::cout << "Warning: could not save calibration image."
                              << std::endl;
                }
            } else {
                std::cout
                    << "No successful checkerboard detection available to save."
                    << std::endl;
            }
        } else if (key == 'c') {
            if (corner_list.size() < 5) {
                std::cout << "Need at least 5 saved calibration frames before "
                             "calibrating."
                          << std::endl;
                continue;
            }
            ...
            bool success = runCameraCalibration(
                point_list, corner_list, frame.size(), cameraMatrix, distCoeffs,
                rvecs, tvecs, reprojectionError);

            calibrationReady = success;
        } else if (key == 'w') {
            if (!calibrationReady) {
                std::cout << "No calibration available yet. Press c first."
                          << std::endl;
                continue;
            }

            bool success =
                writeCalibrationToFile(calibrationResultFile, cameraMatrix,
