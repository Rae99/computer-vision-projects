...
# [LEARN] Standard Python imports. torch/torchvision are the ML framework.
# No web server needed — this is a script you run directly.
# → learning/01-project-structure.md
import sys
import torch
...
# [LEARN] MyNetwork inherits from nn.Module — PyTorch's base class for all networks.
# __init__ declares the layers (like React component state).
# forward() defines how data flows (like render()).
# → learning/03-cnn-architecture.md
# → learning/concepts/cnn-layers.md
class MyNetwork(nn.Module):
    ...
    # [LEARN] forward() is called when you do model(images).
    # Data shape: [batch, 1, 28, 28] → [batch, 10] log-probabilities.
    # 320 = 20 filters × 4×4 spatial size after two conv+pool rounds.
    # → learning/concepts/forward-pass.md
    # → learning/concepts/log-softmax-nllloss.md
    def forward(self, x):
...
# [LEARN] DataLoader is like a paginated API for images.
# transform is middleware: ToTensor (0-255 → 0-1) then Normalize (subtract mean, divide std).
# shuffle=False for test so first 10 are always the same examples.
# → learning/02-data-loading.md
# → learning/concepts/normalization.md
...
# [LEARN] The training loop: 5 steps per batch — zero grad, forward, loss, backward, step.
# model.train() enables dropout; model.eval() (in evaluate()) disables it.
# → learning/04-training-loop.md
# → learning/concepts/training-loop.md
def train_network(model, train_loader, test_loader, epochs=5, lr=0.01):
    # [LEARN] NLLLoss expects log-probabilities (output of log_softmax). Paired by design.
    # → learning/concepts/log-softmax-nllloss.md
...
# [LEARN] This guard means: only run main() if this file is executed directly.
# Other files import MyNetwork/get_data/evaluate without triggering training.
# Equivalent to Node.js: if (require.main === module) { main(); }
# → learning/01-project-structure.md
